...
class CodeCollectorApp:
    ...
    def setup_ui(self):
        ...
        btn1 = ttk.Button(button_frame, text="Скопировать файлы", command=self.collect_and_copy)
        btn1.pack(side=tk.LEFT, padx=5)

        btn2 = ttk.Button(button_frame, text="Скопировать дерево", command=self.copy_tree)
        btn2.pack(side=tk.LEFT, padx=5)

        btn3 = ttk.Button(button_frame, text="Скопировать дерево + файлы", command=self.copy_tree_and_files)
    ...
    def populate_tree(self):
        ...
        """
        root_node = self.tree.insert('', 'end', text=f"{os.path.basename(self.project_path)}", open=True)
        self.tree_items[root_node] = {
        ...
        try:
            for entry in os.listdir(path):
                ...
                if os.path.isdir(entry_path):
                    node = self.tree.insert(parent, 'end', text=f"[DIR] {entry}", open=False)
                    self.tree_items[node] = {
                        'path': entry_path,
                        'type': 'dir'
                    }
    ...
    def on_item_click(self, event):
        ...
        if item['type'] == 'file':
            if item_id in self.selected_files:
                ...
                self.tree.item(item_id, text=self.tree.item(item_id, 'text').replace(" [✓]", ""))
            else:
                # Добавляем в выбор
    ...
    def collect_and_copy(self):
        ...
        self.show_status(f"Содержимое {len(self.selected_files)} файла(ов) скопировано в буфер обмена.", "green")
    ...
    def copy_tree_and_files(self):
        ...
        self.root.update()
        self.show_status(f"Структура проекта + {len(self.selected_files)} файл(ов) скопированы в буфер обмена.", "green")
